			...
			<v-col cols="3" class="w-100 h-100 pa-6">
				<v-card class="w-100 h-100 elevation-10">
					...
					<div class="w-100 pa-5">
						<v-btn class="w-100" color="green" variant="tonal"> Edit Profile </v-btn>
					</div>
				</v-card>
			</v-col>
						...
						<v-card class="w-100 h-100 elevation-6 pa-5">
							<div class="d-flex flex-wrap justify-center">
								<v-chip
									...
									@click="() => {}"
								>
									<span class="text-h6"> Personal Best </span>
									<v-tooltip
										:text="'Calculated as your best score from all your single player submissions'"
								...
								<v-chip
									...
									@click="() => {}"
								>
									<span class="text-h6"> Average WPM </span>
									<v-tooltip
										:text="'Calculated as average over last 10 submissions'"
									...
									<span class="text-h6"> {{ userDetails.averageWPM }} WPM </span>
								</v-chip>
								...
								<v-chip
									...
									@click="() => {}"
								>
									<span class="text-h6"> Rank: 12 </span>
								</v-chip>
								...
								<v-chip
									...
									@click="() => {}"
								>
									<span class="text-h6"> Better then 84% </span>
								</v-chip>
							</div>
						</v-card>
					</v-col>
							...
							<div class="w-100">
								<div class="w-100 d-flex justify-center">
									<v-tabs v-model="tabIndex">
										<v-tab> Single Player </v-tab>
										<v-tab> Multiplayer </v-tab>
									</v-tabs>
								</div>
							</div>
								...
								<v-window class="w-100 h-100" v-model="tabIndex">
									<v-window-item class="w-100 h-100">
										<div class="d-flex w-100 h-100 pa-3">
											...
											<div class="w-66 h-100">
												<v-card
													class="w-100 h-100 d-flex align-center elevation-4"
												>
													<typing-result
														v-if="currSelectedSubmission"
														:line-chart-data="
															currSelectedSubmission.typingSpeedPerSecond
														"
														:raw-line-chart-data="
															currSelectedSubmission.rawTypingSpeedPerSecond
														"
													>
													</typing-result>
												</v-card>
									...
									<v-window-item class="w-100 h-100">
										<div class="d-flex w-100 h-100 pa-3">
											...
											<div class="w-66 h-100">
												<v-card
													class="w-100 h-100 d-flex justify-center align-center elevation-4"
												>
													<v-dialog
...
export default defineComponent({
	...
	setup() {
		...
		const userSubmissionsByDate = computed(() => {
			...
			const sortedByKeys = Object.keys(reducedUserSubmission)
		...
		const datesFrequency = computed(() =>
			userSubmissions.value.reduce((acc, entry) => {
		...
		const fetchUserProfileStats = async () => {
			const call = await fetch(
				`https://canarytype-alpha3.azurewebsites.net/api/UserStats/GetStat?UserName=${userName.value}`,
			);
