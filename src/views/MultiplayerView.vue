			...
			<div v-if="!roomId" class="d-flex w-100 h-100">
				<div class="d-flex w-25 h-100">
					<div class="d-flex w-100 h-100 flex-column align-center justify-center">
						<div>
							<v-icon size="100" icon="mdi-sword"> </v-icon>
						</div>
						<div class="d-flex text-h5 align-center pa-5">
							Play with random person online
						</div>
						<v-btn
							v-if="!isSearchingForOpponent"
							variant="outlined"
							@click="onSearchButtonClicked"
							:disabled="isHostingPrivateRoom || isJoiningPrivateRoom"
						>
							Play Online
						</v-btn>
						...
						<div class="my-5 d-flex flex-column">
							...
							<div v-if="isHostingPrivateRoom && privateRoomIdToShare">
								<div
									class="text-h6 pa-5"
									style="border: 2px solid green; border-radius: 5px"
								>
							...
							<v-btn
								variant="outlined"
								class="my-4"
								@click="onClickPrivateRoom"
								:disabled="isSearchingForOpponent || isJoiningPrivateRoom"
							>
								<div v-if="!isHostingPrivateRoom">Create Private Room</div>
								<div v-else>Cancel</div>
							</v-btn>
							<div
								v-if="isJoiningPrivateRoom"
								class="w-100 h-100 d-flex flex-row align-center justify-center"
							>
								<div class="w-100 d-flex">
									<v-text-field
										class="w-100"
										label="Enter Room Code"
										v-model="privateRoomText"
									>
										<template #append>
											<v-btn
							...
							<v-btn
								variant="outlined"
								@click="onClickJoinPrivateRoom"
								:disabled="isSearchingForOpponent || isHostingPrivateRoom"
							>
								<div v-if="!isJoiningPrivateRoom">Join Private Room</div>
								<div v-else>Cancel</div>
							</v-btn>
						</div>
					</div>
				...
				<div class="w-75 h-100">
					...
					>
						<div class="text-h5">Searching for opponent</div>
		...
		const onClickPrivateRoom = async () => {
			if (isHostingPrivateRoom.value === false) {
				...
				try {
					...
					await conn.value.invoke('CreatePrivateRoom', {
						username: userDetails.uniqueUserName,
					});
				} catch (e) {
					console.log(e);
