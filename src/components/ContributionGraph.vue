							...
							<div class="w-100 h-100" style="padding: 1px" v-for="y in 7" :key="y">
								<v-tooltip :text="getTooltipText(y - 1, x - 1)">
									<template #activator="{ props }">
										<div
											v-bind="props"
											class="w-100 h-100 graph_cell"
											:class="{
												hide_before_jan: hideBeforeJanAfterDec(
													y - 1,
													x - 1,
												),
												one_contribution: highlight5Jan(y - 1, x - 1),
											}"
										></div>
									</template>
								</v-tooltip>
	...
	setup(props) {
		...
		const index = computed(() => {
			...
			for (const date in props.datesContributed) {
				...
				indexArr.push(
					daysInMonths[Number(splitDate[1]) - 1] + Number(splitDate[2] - 1 + offset),
				);
			}
